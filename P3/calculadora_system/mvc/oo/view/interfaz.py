...
class vista:
    ...
    @staticmethod
    def actualizar_screen2(ventana,respuesta,id):
        vista.menuprincipal(ventana)
        vista.borrar_pantalla(ventana)
        lbltit=Label(ventana,text=".::Cambiar una Operacion::.")
        lbltit.pack(pady=5)
        ...
        simb=StringVar()
        res=StringVar()
        lbl_n1=Label(ventana,text="Numero 1:")
        lbl_n1.pack(pady=5)
        ...
        btnenv=Button(ventana,text="Guardar",command=lambda:funciones.funcionesc.enviar_actualizacion(num1.get(),num2.get(),simb.get(),res.get(),respuesta[0]))
        btnenv.pack()
        btnvolv=Button(ventana,text="Volver",command=lambda:vista.interfaz(ventana))
        btnvolv.pack(pady=10)
